...
with open('input', 'r', encoding='cp1047') as file:
    ...
    for line in data:
        ...
        print(last_value)
        # Run through the factors and predict the next number
        # We can skip the 0 factor
        for i in range(len(factors) - 2, -1, -1):
            # Add the previous last value to our last value
            previous_value = factors[i+1][len(factors[i+1])-1]
            this_value = factors[i][len(factors[i]) - 1]
            factors[i].append(this_value + previous_value)
        # Pop any added factors
        for i in range(factors_added):
            factors.pop(len(factors)-1)
        # print("FINAL",factors)
        for factor in factors:
            accumulator += factor[len(factor) - 1]
    print(accumulator)